          ...
          {vulns.map((vuln, vi) => {
            ...
            return (
              <div key={vi} style={{
                ...
                {/* Hint */}
                <p style={{
                  margin: 0, fontSize: "11.5px", color: "#8ba0a6", lineHeight: 1.55,
                }}>
                  💡 {vuln["Hint"]}
                </p>
                {fixed && (
...
// ── Main ──────────────────────────────────────────────────────
export default function GroupRaidClient({ codebaseName, files, filesCode, fileTree }) {
  const [selectedPath,  setSelectedPath]  = useState(files[0]?.Path ?? null);
  const [editedCodes,   setEditedCodes]   = useState(() => ({ ...filesCode }));
  const [progress,      setProgress]      = useState({});   // {[path]:{[cat]:{fixed:[],score:0}}}
